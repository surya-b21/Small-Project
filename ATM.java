...
public class ATM {
		...
		while (exitstatus != true) {
			...
			for (int i = 0; i < user.length; i++) {
				if ((ID.equals(user[i])) && (PIN.equals(pin[i]))) {
					...
					switch (menu) {
					case 1:
						System.out.println("Saldo anda adalah " + saldo[i]);
						break;
					case 2:
	...
	public static int transfer(int saldo[], int i, int j, String user[], String[] norek) {
		int saldopengirim = saldo[i];
		int saldopenerima = saldo[j];
		boolean exit = false;
		while (exit == false) {
